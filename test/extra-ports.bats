...
@test "unset CLAUDE_PORTS: exits 0 with no output" {
  run --separate-stderr bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}

@test "empty CLAUDE_PORTS: exits 0 with no output" {
  run --separate-stderr env CLAUDE_PORTS="" bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
...
@test "default protocol is tcp when no suffix given" {
  run --separate-stderr env CLAUDE_PORTS="6000" bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [[ "$output" == *"/tcp"* ]]
}
...
@test "port 0: rejected as invalid" {
  run --separate-stderr env CLAUDE_PORTS="0" bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}

@test "port 65536: rejected as out of range" {
  run --separate-stderr env CLAUDE_PORTS="65536" bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}

@test "non-numeric port: rejected" {
  run --separate-stderr env CLAUDE_PORTS="abc" bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}

@test "unknown protocol suffix: rejected" {
  run --separate-stderr env CLAUDE_PORTS="3000/sctp" bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}

@test "too many colon fields: rejected" {
  run --separate-stderr env CLAUDE_PORTS="1.2.3.4:8080:3000:extra" bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
...
@test "invalid container port in mapping: rejected" {
  run --separate-stderr env CLAUDE_PORTS="8080:0" bash "${EXTRA_PORTS}"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
