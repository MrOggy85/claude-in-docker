#!/bin/bash
# Smoke-test the docker bridge over Streamable HTTP. Run on the HOST while
# host-docker-bridge.sh is running in another terminal; drives the full MCP
# handshake with curl (no container involved), then checks that an unauthenticated
# request is rejected.
#
# The token identifies the project, so point this at one:
#   ./docker-bridge/smoke-test.sh ~/code/my-project
# With no argument it uses $PWD. Override the port with DOCKER_BRIDGE_PORT.
set -u
...
TOKEN_FILE="$(projects_dir)/$(project_key "${PROJECT_DIR}")/docker-bridge.token"
if [ ! -f "${TOKEN_FILE}" ]; then
  echo "!! no token for ${PROJECT_DIR}"
  echo "   expected: ${TOKEN_FILE}"
  echo "   run \`CLAUDE_DOCKER_BRIDGE=1 ./run.sh\` in that project once to create it."
  exit 1
fi
TOKEN="$(cat "${TOKEN_FILE}")"
PORT="${DOCKER_BRIDGE_PORT:-9334}"
...
SID="$(curl -s --max-time 10 -D - -o "$INIT_BODY" \
...
echo ">> POST tools/list (expect docker_ps, docker_logs, docker_stats)"
curl -sN --max-time 10 \
  -H "$AUTH" -H 'Accept: application/json, text/event-stream' -H 'Content-Type: application/json' \
...
echo ">> POST tools/call docker_ps (expect only allowlisted containers)"
curl -sN --max-time 30 \
  -H "$AUTH" -H 'Accept: application/json, text/event-stream' -H 'Content-Type: application/json' \
  -H "mcp-session-id: $SID" \
  -d '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"docker_ps","arguments":{}}}' "$BASE" \
  | sed 's/^/     /'
