...
| One project's trust/MCP-approval state leaking into another | `claude.json` is seeded and mounted per-project (not shared globally), so an approval recorded for one project can't apply to an unrelated one that happens to run through this tool later — see [Shared `claude.json` Collapses Per-Project Trust State](attack-vectors.md#shared-claudejson-collapses-per-project-trust-state-mitigated). |
