...
| `CLAUDE_ALLOW_PROJECT_SETTINGS` | _(unset)_ | Accepts `1`/`true`/`yes`/`on`. Skips the project-settings guard entirely and proceeds with a project-level `.claude/settings.json` as-is. | [Known Attack Vectors](attack-vectors.md#project-level-claude-settings-mitigated-by-default) |
| `CLAUDE_PROJECT_SETTINGS_STRICT` | _(unset)_ | Accepts `1`/`true`/`yes`/`on`. Reviews the **whole** settings file every run — the pre-scanner behaviour — ignoring both the capability scan and any approval you recorded. For auditing. | [Known Attack Vectors](attack-vectors.md#project-level-claude-settings-mitigated-by-default) |
...
| `CLAUDE_DOCKER_BRIDGE` | _(unset)_ | Accepts `1`/`true`/`yes`/`on`. Enables read-only host Docker access: mints the per-project bridge token, forwards it as `DOCKER_BRIDGE_TOKEN`, and opens `DOCKER_BRIDGE_PORT` outbound. Off by default. | [Host Docker Bridge](docker-bridge.md) |
