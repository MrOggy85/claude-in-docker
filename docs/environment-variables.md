...
| `CHROME_DEVTOOLS_MCP_PORT` | `9333` | Port the `chrome-devtools-mcp` bridge listens on. Read by the host bridge, **not** `run.sh` — so unlike `SOUND_PORT` it is not auto-merged into the firewall; open it with `CLAUDE_HOST_OUTBOUND_PORTS="9333"`. | [Chrome DevTools MCP](chrome-devtools-mcp.md) |
| `CHROME_DEVTOOLS_MCP_EXTRA_ARGS` | _(unset)_ | Extra flags the host bridge passes to `chrome-devtools-mcp` (e.g. `--channel canary`). Host-only. | [Chrome DevTools MCP](chrome-devtools-mcp.md) |
