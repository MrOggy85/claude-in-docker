...
- `muted-hosts.txt` hosts the [egress alert watcher](docs/egress-alerts.md) never notifies about. It changes no decision — a muted host stays denied, silently — which is the answer to telemetry you cannot turn off at the source. Edit with `./cid mute add|rm <host>`.
...
- [Chrome DevTools MCP](docs/chrome-devtools-mcp.md) — drive **your own** Chrome on the host from the container, bridged over HTTP with a per-project token and a persistent per-project profile (`CLAUDE_CHROME_DEVTOOLS=1`); for project work prefer the in-container browser below, and reach for this when the task needs your real profile, a host-only service, or your own window
- [In-container browser](docs/browser-vnc.md) — a real Chromium and `playwright-cli` inside the container, so browser traffic crosses the egress proxy and screenshots land in the repo; the browser gets its own additive allowlist (`cid domains --browser`), so a CDN you allow to render a page never widens what the agent's own `curl` can reach; watch it live over noVNC with `cid vnc` (`CLAUDE_BROWSER=1`); off by default
